...
data = pd.read_csv('Data_Red_Null_Tra.csv')
#raw_data = data.copy()
...
for f_ in categorical_feats:
    ...
    train_features = [f for f in x_train if f not in ['Target','ID']]
...
for f_ in categorical_feats:
    ...
    train_features = [f for f in x_test if f not in ['Target','ID']]
    # Shuffle target if required
...
shap.dependence_plot("EXT1", shap_values, features)
